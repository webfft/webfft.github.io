import { fft_1d } from './webfft.js';
import { conjugate, bluestein_fft_1d } from './webfft_ext.js';
...
for (let x of [x1M]) {
  ...
  let y2 = new Float32Array(x);
  time('fft_1d 1M', () => fft_1d(y1));
  time('bluestein_fft_1d 1M', () => bluestein_fft_1d(y2, y2));
  console.log('RMSE:', Math.abs(1 - dot(y1, y1) / dot(y2, y2)).toExponential(2));
}
